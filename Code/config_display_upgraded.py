...
def config_popup(config):
    ...
    label_width = 360  # Width of the labels 
    entry_width = 120  # Width of the entry fields 
    window_width = label_width + entry_width + 3 * padding
    window_height = (num_options + 5) * 30 + 15  # Assuming each option takes 30 pixels in height
    ...
    def select_directory(key):
        selected_directory = filedialog.askdirectory()
        if selected_directory:
            config[key] = selected_directory
            entries[key].delete(0, tk.END)
            entries[key].insert(0, selected_directory)
    ...
    for key, value in config.items():
        ...
        entries[key] = entry
        if key in ['root', 'mapping_root', 'population_root', 'manual_reference_root', 'prac10yrage_root']:
            directory_entry = entry
            directory_button = tk.Button(window, text="Open", command=lambda k=key: select_directory(k))
            directory_button.grid(row=row, column=2, padx=10, pady=5)
    ...
    def continue_clicked():
        ...
        for key, entry in entries.items():
            if key in ["test_run", "rerun_pbi", "use_corporate_reference", "override_run_LSOA","Exclude_unknown_gender"]:
     ...
     # Update the config.json file with the new configuration values
        ...
        messagebox.showinfo("Config Menu", "Updated config menu, Continuing...")
